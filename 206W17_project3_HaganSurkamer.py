...
table_spec += 'text TEXT, time_posted TIMESTAMP, retweets INTEGER, user_id TEXT NOT NULL, FOREIGN KEY (user_id) REFERENCES Users(user_id) ON UPDATE SET NULL)'
...
table_spec1 += 'Users (user_id INTEGER PRIMARY KEY, '
table_spec1 += 'screen_name TEXT, num_favs INTEGER, descriptions TEXT)'
...
for elem in umich_tweets:
    statement = 'INSERT OR IGNORE INTO Tweets VALUES (?, ?, ?, ?, ?)'
    cur.execute(statement, (elem['id_str'], elem["text"], elem["created_at"], elem["retweet_count"], elem['user']['id_str']))
...
q4 = 'SELECT descriptions FROM Users WHERE num_favs >= 25'
cur.execute(q4)
descriptions_fav = cur.fetchall()
descriptions_fav_users = [x[0] for x in descriptions_fav]
